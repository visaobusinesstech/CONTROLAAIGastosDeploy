/**
 * Rota Vercel: GET /auth/me → /api/auth/me — quem está logado?
 *
 * O que faz: lê o JWT do header Authorization, valida assinatura e token_version,
 * busca usuário no Postgres e devolve nome, e-mail, plano e nível de acesso.
 *
 * Quando o usuário usa: ao abrir o painel, refresh de sessão ou após F5 — React chama
 * esta rota para saber se ainda está autenticado.
 *
 * Por que roda na Vercel: sessão web fica no mesmo domínio do login; evita proxy
 * desnecessário ao Railway para uma consulta leve e frequente.
 *
