/**
 * Rota Vercel: POST /auth/login → /api/auth/login — entrada no sistema
 *
 * O que faz: valida e-mail/senha no Postgres (bcrypt), incrementa token_version se
 * necessário e emite JWT de 7 dias; se 2FA estiver ativo, retorna challenge OTP em vez do token.
 *
 * Quando o usuário usa: formulário /login no React; resposta alimenta AuthProvider.
 *
 * Por que roda na Vercel: credenciais nunca passam pelo browser além do POST; JWT é
 * gerado na edge com JWT_SECRET e DATABASE_URL do painel Vercel.
 *
