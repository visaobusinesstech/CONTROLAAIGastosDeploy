 ...
 * Cobre /api/auth/login|me|forgot|reset|2fa/* via catch-all.
 */
...
function pathKey(req: VercelRequest): string {
  ...
  // Fallback se o rewrite chegar sem query.path (pathname /api/auth/...)
  const url = req.url ?? "";
  const idx = url.indexOf("/api/auth/");
  if (idx >= 0) {
    return url
      .slice(idx + "/api/auth/".length)
      .split("?")[0]
      .replace(/^\/+|\/+$/g, "");
  }
  return "";
